    ...
    return (
        ...
        <div className="w-full h-[660px] bg-cover bg-center flex items-center justify-center bg-[url(../assets/images/Empower.png)]">
            <div className="text-center text-black max-w-3xl px-4">
                <p className="text-[54px] font-[500] leading-[61px] text-white">From Ideas to Standing Ovations!</p>
                <p className="text-[24px] font-[400] leading-[30px] text-white">
                We prepare kids to pitch like pros – with confidence, creativity, and a plan!
                </p>
            </div>
        </div>
        <div 
            ...
            <div className="relative z-10 flex flex-col md:flex-row h-full">
                <div className="w-full md:w-1/2 flex items-center pl-4 md:pl-16">
                    <div className="text-white">
                        <h2 className="text-[36px] md:text-[54px] font-[500] leading-[40px] md:leading-[61px] mb-4 mt-10">Ready to Empower Your Child?</h2>
                        <p className="text-[18px] md:text-[24px] font-[400] leading-[24px] md:leading-[30px] mb-6 max-w-lg">
                        Sign up for a free personal trial class and experience the difference.
                        </p>
                    </div>
                </div>

                <div className="w-full md:w-1/2 flex justify-end pr-4 md:pr-16">
                    <div className="bg-white rounded-[30px] shadow-lg p-6 mt-10 mx-auto" style={{ height: '372px', width: '100%', maxWidth: '364px' }}>
                        <h3 className="text-lg font-semibold mb-4 text-black">Select date</h3>
                        <div className="grid grid-cols-2 gap-2">
                            {['Tomorrow', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'].map((day, index) => (
                                <button
                                    key={index}
                                    className={`border rounded-lg p-2 text-black ${selectedDate === day ? 'bg-purple-200' : ''}`}
                                    style={{ borderColor: 'rgba(217, 217, 217, 1)' }}
                                    onClick={() => handleButtonClick(day)}
                                >
                                    {day}<br />{index + 12} March
                                </button>
                            ))}
                        </div>
                        <a href="/" className="bg-[#7F00FF] text-white text-[16px] font-[700] leading-[24px] w-full py-2 rounded-lg mt-4 text-center block">
                            Book a Free Class!
                        </a>
                    </div>
                </div>
