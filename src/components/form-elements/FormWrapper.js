    ...
    <div className="grid grid-cols-1 sm:grid-cols-5  gap-x-0 gap-y-8 sm:gap-8 mx-6 my-6 sm:my-0 sm:mx-0">
      <div className="bg-white col-span-2 pt-12 pb-8  px-6 rounded-lg">
        ...
        {children}
          ...
          <p className={`text-black text-center font-medium text-sm pt-6 ${note ? 'opacity-100' : 'opacity-0'}` }>
            Note: {note}
          </p>
        
        
          <p className={`text-black text-center font-sans text-xs pt-6 ${secondryNote1 ? 'opacity-100' : 'opacity-0'}`}>
            {secondryNote1}
          </p>
        
        
          <p className={`text-black text-center font-sans text-xs ${secondryNote2 ? 'opacity-100' : 'opacity-0'}`}>
By proceeding further, you agree to our <a href="#" className="underline">Terms & Conditions </a>and our <a href="#" className="underline">Privacy Policy</a>.
          </p>
      ...
      <div className="col-span-3">
        ...
        {pointersData?.map((item, index) => (
          <div key={index} className="flex gap-2 mb-4">
            ...
            <div className="text-lg sm:text-2xl leading-6 sm:leading-8">{item?.value}</div>
          </div>
        ))}
      </div>
    </div>
